...
- skeleton 主导的连续开发可以使用复合 issue 分支，例如 `skeleton/CTX-01-MEM-01-context-memory-foundation`。
...
- 分支名包含开发者前缀和所有主要 issue 编号，例如 `skeleton/CTX-01-MEM-01-context-memory-foundation`。
- 只合并强依赖、同一数据链路、或连续验收更高效的 issue。
- 每个 commit 和开发日志仍要说明对应 issue 范围。
- PR 描述必须列出包含的 issue、验证结果、不做范围和遗留风险。
- 如果复合分支开始变得难以 review，应停止继续叠加，拆出后续分支。
